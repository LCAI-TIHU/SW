...
def run_inference(mod):
    ...
    for i, batch in enumerate(val_data):
        ...
        pred = prediction[0].argmax(axis=0)
        print(i, pred, label, pred == label)
        if pred == label:
            top1_cnt = top1_cnt + 1
        if i >= total:  # only run inference on a few samples in this tutorial
            print("accuracy=", top1_cnt / total)
            break
...
# -------------
# We create a Relay VM to build and execute the model.
def run_original_inference(mod, params):
    ######################################################################
    # Compile the Graph
    # -----------------
...
def main():
    ...
    run_original_inference(mod, params)
