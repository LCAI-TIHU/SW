...
def build_module(mod, target, params=None, enable_acl=True, tvm_ops=0, acl_partitions=1):
    ...
    with tvm.transform.PassContext(opt_level=3, disabled_pass=["AlterOpLayout"]):
        if enable_acl:
        # if False:
            print("befor merge mod:",mod["main"])
            mod_external = arm_compute_lib.partition_for_arm_compute_lib(mod, params)
            '''mod_main_noexternal = mod
            print("arm mod,",mod_main_noexternal)
            func_main_external = set_external_func_attr(mod_main_noexternal["main"], "aipu", "aipu_1")
            mod_external = tvm.IRModule()
            mod_external["main"] = func_main_external'''
            print("external codege aipu:",mod_external["main"])
        ...
        # return relay.build(mod_external, target=target, params=params)
