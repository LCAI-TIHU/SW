...
def update_lib(lib):
    ...
    print("contrib path:",contrib_path)
    kwargs = {}
...
def test_multi_node_subgraph():
    ...
    subgraph0 = set_external_func_attr(subgraph0, "aipu", "aipu_0")
    ...
    subgraph1 = set_external_func_attr(subgraph1, "aipu", "aipu_1")
    ...
    r = relay.add(call0, call1)
    f = relay.Function([x, w0, w1, w2, w3, w4, w5], r)
    mod = tvm.IRModule()
    mod["main"] = f
    mod = relay.transform.InferType()(mod)
    print("mod mod:",mod)
    new_mod_main = set_external_func_attr(mod["main"], "aipu", "aipu_2")
    new_mod = tvm.IRModule()
    new_mod["main"] = new_mod_main
    print("newmod mod:",new_mod)
...
def test_extern_aipu():
    if not tvm.get_global_func("relay.ext.aipu", True):
        print("skip because aipu codegen is not available")
        return
    ...
    f = set_external_func_attr(f, "aipu", "aipu_0")
    call = relay.Call(f, [data0, weight0, weight0])

    mod_main_noexternal = tvm.IRModule.from_expr(call)
    print("external codege aipu:",mod_main_noexternal["main"])
    func_main_external = set_external_func_attr(mod_main_noexternal["main"], "aipu", "aipu_1")
    mod = tvm.IRModule()
    mod["main"] = func_main_external
    print("external codege aipu:",mod["main"])
    i_data = np.random.uniform(0, 1, ishape).astype(dtype)
    w_data = np.random.uniform(0, 1, w1shape).astype(dtype)
...
def test_extern_aipu_func_call():
    if not tvm.get_global_func("relay.ext.aipu", True):
        print("skip because aipu codegen is not available")
        return
    ...
    weight2 = relay.var("weight2", shape=(w1shape), dtype=dtype)

    data00 = relay.var("data00", shape=(ishape), dtype=dtype)
    weight01 = relay.var("weight01", shape=(w1shape), dtype=dtype)
    weight02 = relay.var("weight02", shape=(w1shape), dtype=dtype)
    depthwise_conv2d_1 = relay.nn.conv2d(
        data00, weight01, kernel_size=(3, 3), padding=(1, 1), groups=32
    )
    depthwise_conv2d_2 = relay.nn.conv2d(
        depthwise_conv2d_1, weight02, kernel_size=(3, 3), padding=(1, 1), groups=32
    )
    out = relay.add(depthwise_conv2d_1, depthwise_conv2d_2)
    
    f = relay.Function([data00, weight01, weight02], out)
    #ref_mod = tvm.IRModule()
    #ref_mod["main"] = f

    f = set_external_func_attr(f, "aipu", "aipu_0")
    call = relay.Call(f, [data0, weight0, weight1])
    
    weight11 = relay.var("weight11", shape=(w1shape), dtype=dtype)
    out = relay.nn.conv2d(call,weight11, kernel_size=(3, 3), padding=(1, 1), groups=32)
    f_new = relay.Function([data0, weight0, weight1,weight2], out)
    f_new = set_external_func_attr(f_new, "aipu", "aipu_1")
    mod = tvm.IRModule()
    mod["main"] = f_new
    print("mod aipu0 aipu1:",mod)
    i_data = np.random.uniform(0, 1, ishape).astype(dtype)
    w_data = np.random.uniform(0, 1, w1shape).astype(dtype)
