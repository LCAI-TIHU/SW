...
from tvm.relay.op.contrib.arm_compute_lib import *
...
def check_result(
    ...
    def check_graph_executor_result():
        print("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx Before relay.build xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx")
        with tvm.transform.PassContext(opt_level=3, disabled_pass=["AlterOpLayout"]):
...
def test_extern_dnnl_mobilenet():
    ...
    print("print mobile mod:",mod)
    mod["main"] = relay.build_module.bind_params_by_name(mod["main"], params)
    #func_main_external = set_external_func_attr(mod["main"], "aipu", "aipu_1")
    #mod["main"] = func_main_external
    with tvm.transform.PassContext(opt_level=3, disabled_pass=["AlterOpLayout"]):
        #module = relay.build(mod,"llvm")
        ...
        mod = seq(mod)
        print("merge composite mod:",mod)
        module = relay.build(mod,"llvm")
        #mod = transform.MergeComposite(arm_compute_lib_pattern_table())(mod)
...
def test_multiple_ends():
    ...
    def pattern(expr):
        # print(expr.args[0].op.name)
        if expr.args[0].op.name == "sigmoid":
            return True
        return False
    _ffi_api.RegisterOpAttr("abs", "target.test", pattern, level)
    ...
    for annotate_non_call_ops in [False, True]:
        ...
        mod = relay.transform.MergeCompilerRegions()(result)
