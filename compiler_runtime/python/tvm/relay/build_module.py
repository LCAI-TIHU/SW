...
operator_list = ["nn.conv2d"]
#operator_list = ["nn.softmax"]
#operator_list = ["nn.nms"]
        ...
        for arg in expr.args:
            if isinstance(arg, Call):
                ...
                if isinstance(arg.tuple_value, Call):
                    ...
                    if arg.tuple_value.op.name not in operator_list:
                        tuple_value = compiler_end(tuple_value, "default")
                        tuple_value = compiler_begin(tuple_value, "default")
                        arg = TupleGetItem(tuple_value, arg.index)
                        arg = compiler_end(arg, "default")
                        arg = compiler_begin(arg, "default")
                    else:
...
"""def annotate(anno, fn):
    ...
    if new_fn.body.op.name not in operator_list:
        new_body = compiler_end(new_fn.body, "default")
    else:
        new_body = compiler_end(new_fn.body, anno.compiler)
    return Function(list(new_params), new_body, new_fn.ret_type, new_fn.type_params, new_fn.attrs)"""
...
def _build_module_no_factory(mod, target=None, target_host=None, params=None, mod_name="default"):
    ...
    print("In _build_module_no_factory before return build")
...
def build(ir_mod, target=None, target_host=None, params=None, mod_name="default"):
    ...
    params : dict
        The parameters of the final graph.
    """
    # do the annotation firstly
    # print(type(target))
    # print(target)
    # operator_list = ["nn.softmax"]
    """if target != "llvm":
        func = ir_mod["main"]
        global_var = ir_mod.get_global_var("main")
        anno = CustomizedAnnotation("dnnl")
        func = annotate(anno, func)
        func = run_opt_pass(func, transform.MergeCompilerRegions())
        print("Before partitiongraph ....................")
        print(func)
        ir_mod = IRModule.from_expr(func) 
        ir_mod = transform.PartitionGraph()(ir_mod)
        print("After partitiongraph .....................")
        print(ir_mod["main"])"""
    """if target == "llvm":
        func = ir_mod["main"]
        #global_var = ir_mod.get_global_var("main")
        anno = CustomizedAnnotation("aipu")
        func = annotate(anno, func)
