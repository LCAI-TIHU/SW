...
def aipu_partition():
    ...
    ret: tvm.transform.Pass
        The registered pass for VTA rewrite.
    """
    return _quantize.QuantizeAipuPartition()
...
def aipu_annotate():
    ...
    ret: tvm.transform.Pass
        The registered pass for quantization annotation.
    """
    return _quantize.QuantizeAipuAnnotate()
...
def prerequisite_optimize(mod, params=None):
    ...
    optimize = tvm.transform.Sequential(
        [
            _transform.EliminateCommonSubexpr(),
...
def quantize(mod, params=None, dataset=None):
    ...
    if (q_cfg.target=="aipu"):
        quant_passes = [aipu_partition(), aipu_annotate(), calibrate_pass, tvm.relay.transform.InferType()]
        required_pass = ["QuantizeAipuPartition", "QuantizeAipuAnnotate", "QuantizeCalibrate"]
    else:
    ...
    with tvm.transform.PassContext(opt_level=3, required_pass=required_pass):
        with quantize_context():
            mod = quantize_seq(mod)

    print("-------------pose quantize model--------------")
    print(mod["main"].astext(show_meta_data=False))
