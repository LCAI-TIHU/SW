...
def _kl_scale(mod, dataset):
    ...
    for samples in collect_stats(mod, dataset, chunk_by):
        ...
        scales = [_find_scale_by_kl(sample) for sample in samples] #Abraham To walk around error: OverflowError: cannot serialize a bytes object larger than 4 GiB
...
def _find_scale_by_percentile(arr, percentile=0.9999):
...
def _percentile_scale(mod, dataset):
    ...
    for samples in collect_stats(mod, dataset, chunk_by):
        ...
        scales = [_find_scale_by_percentile(sample) for sample in samples]
        # with mp.Pool() as pool:
...
def _set_params(mod, input_scale_func, weight_scale_func):
    ...
    def visit_func(expr):
        ...
        if isinstance(expr, _expr.Call) and expr.op == quantize_op:
            ...
            const_params[ndom_scale] = _make_const(scale / (valid_range - 1))
            const_params[nclip_min] = _make_const(-(valid_range))
    ...
    #Abraham do this for https://discuss.tvm.apache.org/t/pytorch-quantization-error-during-quantization/7167/5
...
def _max_scale(sq_call):
    ...
    assert isinstance(var, _expr.Constant)
    val = np.amax(np.abs(var.data.asnumpy()))
    print("_max_scale val: ", val, var.data.asnumpy().shape)
...
def _per_channel_max_scale(sq_call):
    """calculate weight per channel scale with maximum absolute value"""
    var = sq_call.args[0]
    assert isinstance(var, _expr.Constant)

    axis = sq_call.attrs["axis"]
    if axis < 0:
        axis = axis + var.data.asnumpy().ndim
    
    if axis == 3:
        val = np.amax(np.abs(var.data.asnumpy()), axis=(0, 1, 2))
    elif axis == 2:
        val = np.amax(np.abs(var.data.asnumpy()), axis=(0, 1))  # Jasper: Depthwise op
    elif axis == 0 and var.data.asnumpy().ndim == 2: # Jasper: dense op
        val = np.amax(np.abs(var.data.asnumpy()), axis=1)
    else: 
        val = var.data.asnumpy()

    # print("_per_channel_max_scale val: ", val)
...
def calibrate(dataset=None):
    ...
    def wrapped_func(mod, _):
        ...
        elif cfg.weight_scale == "channel_max":
            weight_scale_func = _per_channel_max_scale
