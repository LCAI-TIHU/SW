...
from tvm.runtime import _ffi_api as _runtime_ffi_api # shenfw add
from tvm.rpc import base as rpc_base
from tvm._ffi.base import string_types
from tvm._ffi.runtime_ctypes import Device
from tvm.runtime import ndarray
#from tvm._ffi._cython import base as cython_base
...
class GraphModule(object):
    ...
    def __init__(self, module):
        # print("In python GraphModule __init__ ...")
        # print(module)
        # print(len(module.imported_modules))
        self.module = module
    ...
    def set_input(self, key=None, value=None, **params):
        ...
        """
        # print("In python set_input ...")
        #------------shenfw add begin -----------#
        #target_name = "llvm"
        # print(len(self.module.imported_modules))
        #print("the type of the value: ", type(ndarray.numpyasarray(value.asnumpy()))) 
        #if target_name == "llvm":
        #    _runtime_ffi_api.set_input(value)
        if len(self.module.imported_modules)!=0:
             _runtime_ffi_api.set_input(self.module.imported_modules[0], key, value)
        else:
            if key is not None:
                # print("key is not None ", input)
                v = self._get_input(key)
                if v is None:
                    raise RuntimeError("Could not find '%s' in graph's inputs" % key)
                v.copyfrom(value)
        #------------shenfw add end -----------#
        """if key is not None: # shenfw comment out
            print("key is not None ", input)
            v = self._get_input(key)
            if v is None:
                raise RuntimeError("Could not find '%s' in graph's inputs" % key)
    ...
    def run(self, **input_dict):
        ...
        """
        #------------shenfw add begin -----------#
        # print("The module imports number is ", len(self.module.imported_modules))
        #target_name = "llvm"
        #if target_name == "aipu":
        if len(self.module.imported_modules)!=0:
             _runtime_ffi_api.run(self.module.imported_modules[0])
        else:
            if input_dict:
                self.set_input(**input_dict)
            self._run()
        #------------shenfw add end -----------#
        """if input_dict: # shenfw comment out
            self.set_input(**input_dict)
        self._run()"""
    ...
    def get_output(self, index, out=None):
        ...
        """
        # ---------shenfw add begin
        # print("In python get_output")
        if len(self.module.imported_modules)!=0:
            _runtime_ffi_api.get_output(self.module.imported_modules[0], index, out)
            #print(out)
            return out
            #return _runtime_ffi_api.get_output(self.module.imported_modules[0])
        else:
            if out:
