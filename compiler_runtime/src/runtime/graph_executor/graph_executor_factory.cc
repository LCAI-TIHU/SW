...
#include "../contrib/aipu/aipu_runtime.h" // shenfw add
...
using namespace tvm::runtime::contrib; // shenfw add
namespace tvm {
namespace runtime {
...
PackedFunc GraphExecutorFactory::GetFunction(
  ...
  if (name == module_name_) {
    // LOG(INFO)<<"the module_name is "<< name <<" in graph executor factory get function xxxxxxxxxxxxxxxxxxxxxxxxxxxxx";
    return PackedFunc([sptr_to_self, this](TVMArgs args, TVMRetValue* rv) { // return a packed function definition
...
Module GraphExecutorFactory::ExecutorCreate(const std::vector<Device>& devs) {
  auto exec = make_object<GraphExecutor>(); // Final the object is graph executor!
  // LOG(INFO)<<"c++ in graphexecutorfactory::executorcreate ------------------------";
  //------------------shenfw add start-------------------
  if (devs[0].device_type == 16){

      Module mod = this->imports_[0];
      auto host_mod = Module(mod->imports()[0]);
      auto aipu_mod = Module(mod->imports()[1]);

      std::string ext_runtime_name = "tvm.nvdla_runtime_create"; 
      std::vector<TVMValue> values(4);
      ...
      for (const auto& dev : devs) {
          ...
          setter(i, dev_type);
          setter(i+1, dev.device_id);
          // LOG(INFO)<<"the dev_type ["<< i << "] = " << dev.device_type;
          // LOG(INFO)<<"the dev.device_id ["<< i << "] = " << dev.device_id;
	  i = i + 2;
      }
      TVMArgs args = TVMArgs(values.data(), codes.data(), 4);
      // LOG(INFO)<<"the args number is: "<<args.num_args;
      auto pf = tvm::runtime::Registry::Get(ext_runtime_name);
      // LOG(INFO)<<"After get the pf";
      ICHECK(pf) << "Failed to find the runtime for " << ext_runtime_name << "\n";
      aipu_mod = (*pf)(args[0], args[1], args[2], args[3]);

      // aipu runtime create
      //n->Import(args[1]);
      //size = aipu_mod->GetBuffSize();
      //LOG(INFO)<<"The buffer size is: "<<size;
      //riscv_source = aipu_mod->GetSource();
      //LOG(INFO)<<"The riscv source is: "<<riscv_source;
      //runtime::Module m = n->AipuRuntimeCreate(args[0], args[1], devs); //, PackedFunc());


      // LOG(INFO)<<"Before host runtime create";
      // host runtime create
      //std::vector<Device> host_devs;
      //host_devs[0].device_type = kDLCPU;
      //host_devs[0].device_id = 0;
      //exec->Init(args[0], host_mod, host_devs, PackedFunc()); 
      // set params
      //SetParams(exec.get(), this->params_);
      //LOG(INFO)<<"The mod imports size is:"<<n->imports().size();
      //TestInfo NvDla_testInfo;
      //NvDla_testInfo.numInputs = 4;
      //static_cast<AIPUModuleNode*>(Module(aipu_mod).operator->())->SetNvDla_testInfo(NvDla_testInfo);
      exec->Import(aipu_mod);
      exec->Import(host_mod);
      // LOG(INFO)<<"The exec imports size is:"<<exec->imports().size();
      // LOG(INFO)<<"After the exec Import ......";
      return Module(exec);

  } //------------------shenfw add end
  /*else{
      LOG(INFO)<<" this->imports_ size is: "<<(this->imports_).size();
      int i = 1;
      for (const auto& dev : devs) {
          LOG(INFO)<<"the dev_type ["<< i << "] = " << dev.device_type;
          LOG(INFO)<<"the dev.device_id ["<< i << "] = " << dev.device_id;
...
TVM_REGISTER_GLOBAL("tvm.graph_executor_factory.create")
    .set_body([](TVMArgs args, TVMRetValue* rv) {
      ...
      // LOG(INFO)<<"The args size is: ------------------------ "<<args.size();
      std::unordered_map<std::string, tvm::runtime::NDArray> params;
      ...
      // LOG(INFO)<<"c++ in graph_executor_factory.create --- before Import ...";
      exec->Import(args[1]);
      *rv = Module(exec);
      // LOG(INFO)<< "*************GRAPH FACTORY DEBUG ***************";
    });
