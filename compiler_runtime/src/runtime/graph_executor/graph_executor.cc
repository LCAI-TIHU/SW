...
void GraphExecutor::Init(const std::string& graph_json, tvm::runtime::Module module,
  ...
  // LOG(INFO)<<"Before SetupStorage ...";
  this->SetupStorage();
  this->SetupOpExecs();
  // LOG(INFO)<<"After SetupOpExecs ...";
...
void GraphExecutor::CopyOutputTo(int index, DLTensor* data_out) {
  ...
  for (int32_t j = 0; j < data->ndim; ++j) {
    ...
    //printf("The shape of the data_out = %d\n", data_out->shape[j]);
  }

  data_entry_[eid].CopyTo(data_out);
  for (int32_t k = 0; k < data_out->shape[1]; k++) {
    printf("The value of the data_out before = %d\n", ((int*)data_out->data)[k]);
    //((int*)data_out->data)[k] = 9 - k;
    //printf("The value of the data_out after = %d\n", ((int*)data_out->data)[k]);
  }
}
...
Module GraphExecutorCreate(const std::string& sym_json, const tvm::runtime::Module& m,
                           const std::vector<Device>& devs,
                           const PackedFunc lookup_linked_param_func) {
  // LOG(INFO)<<"In GraphExecutorCreate !";
...
std::vector<Device> GetAllDevice(const TVMArgs& args, int dev_start_arg) {
  ...
  for (int i = dev_start_arg; i < args.num_args; i += 2) {
    ...
    // LOG(INFO)<<"the args "<<(i+1);
    // LOG(INFO)<<"the device_id = "<<dev.device_id;
    ret.push_back(dev);
  }
  return ret;
}
...
TVM_REGISTER_GLOBAL("tvm.graph_executor.create").set_body([](TVMArgs args, TVMRetValue* rv) {
  ICHECK_GE(args.num_args, 4) << "The expected number of arguments for graph_executor.create is "
                              ...
                              << args.num_args;
  // LOG(INFO)<<"the TVMArgs args num is:"<<args.num_args;
  ...
  std::string ext_runtime_name = "tvm.nvdla_runtime_create"; // shenfw add
  // LOG(INFO)<<ext_runtime_name;//shenfw add
  // LOG(INFO)<<devices[0].device_type;
  if (devices[0].device_type == 16){
  	auto pf = tvm::runtime::Registry::Get(ext_runtime_name);
  	ICHECK(pf) << "Failed to find the runtime for " << ext_runtime_name << "\n";
  	runtime::Module ext_runtime_mod = (*pf)(args[0], args[1], args[2], args[3]);
  }
  else{
