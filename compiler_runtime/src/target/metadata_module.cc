...
runtime::Module CreateMetadataModule(
  ...
  // LOG(INFO)<<"In src target metadatamodule ...";
  ...
  for (tvm::runtime::Module mod : ext_modules) {
    // LOG(INFO)<<"In src target metadatamodule FOR LOOP for ext_modules ...";
  ...
  if (is_targeting_crt) {
    // LOG(INFO)<<"In src target metadatamodule is targeting crt ...";
    ...
    if (!non_crt_exportable_modules.empty()) {
      ...
      // LOG(INFO)<<"In src target metadatamodule Import target_module ...";
      for (const auto& it : non_crt_exportable_modules) {
        binary_meta_mod.Import(it);
	// LOG(INFO)<<"In src target metadatamodule Import external modules ...";
      }
      return binary_meta_mod;
    }
  }
  // LOG(INFO)<<"In src target metadatamodule before return ...";
