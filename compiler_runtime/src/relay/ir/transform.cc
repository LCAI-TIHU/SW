...
IRModule FunctionPassNode::operator()(IRModule mod, const PassContext& pass_ctx) const {
  ...
  // LOG(INFO) << "Executing function pass : " << pass_info->name
  ...
  for (const auto& it : updated_mod->functions) {
    // LOG(INFO)<<"pass name:"<<pass_info->name<<",base func name:"<<it.first;
    // only picks up relay::Function
    if (auto* n = it.second.as<FunctionNode>()) {
      Function func = GetRef<Function>(n);
      //LOG(INFO)<<"before pass:"<<AsText(func,false);
      bool skipfunc = SkipFunction(func);
      // LOG(INFO)<<"pass name:"<<pass_info->name<<",func name:"<<it.first<<","<<skipfunc;
      auto updated_func = skipfunc ? func : pass_func(func, updated_mod, pass_ctx);
      //LOG(INFO)<<"after pass:"<<AsText(updated_func,false);
