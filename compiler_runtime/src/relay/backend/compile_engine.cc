 ...
 public:
  ...
  PackedFunc JIT(const CCacheKey& key) final {
    ...
    if (const auto* f = runtime::Registry::Get("relay.backend.build")) {
      // LOG(INFO)<<"prebuilt relay.backend.end";
      m = (*f)(value->cached_func->funcs, key->target);
    } else {
      // LOG(INFO)<<"prebuilt built";
  ...
  Array<tvm::runtime::Module> LowerExternalFunctions() {
    ...
    // LOG(INFO)<<"xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx In Lower External Functions xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"; 
    for (const auto& it : cache_) {
      ...
      if (src_func->GetAttr<String>(attr::kCompiler).defined()) {
        ...
        // LOG(INFO)<<"the ext_name is: "<<ext_name;
        auto pf = tvm::runtime::Registry::Get(ext_name);
        // LOG(INFO)<<"After get the ext function ......";
  ...
  Array<tvm::runtime::Module> LowerAipuFunctions(relay::Function func) {
    Array<tvm::runtime::Module> ret;
    std::unordered_map<std::string, std::string> cached_symbol;
    auto src_func = func;
    ICHECK(src_func.defined());
...
TVM_REGISTER_GLOBAL("relay.backend._CompileLowerAipuFunctions")
    .set_body_typed([](CompileEngine self,relay::Function func) { return self->LowerAipuFunctions(func); });
