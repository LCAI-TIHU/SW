...
class GraphExecutorCodegen : public backend::MemoizedExprTranslator<std::vector<GraphNodeRef>> {
 public:
  ...
  LoweredOutput Codegen(relay::Function func) {
    ...
    // LOG(INFO)<<"lower external functions end";
    //ret.external_mods = compile_engine_->LowerAipuFunctions(func);
    //LOG(INFO)<<"lower aipu external functions end";
    return ret;
  }
 ...
 protected:
  ...
  std::vector<GraphNodeRef> VisitExpr_(const CallNode* op) override {
    ...
    if (!func->HasNonzeroAttr(attr::kPrimitive)) {
      LOG(INFO)<<"FUNC ERROR:"<<AsText(func,false);
      //return {};
