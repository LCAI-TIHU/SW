...
bool SimulatedQuantizeRel(const Array<Type>& types, int num_inputs, const Attrs& attrs,
  ...
  if(cfg->weight_scale == "channel_max" && param->kind == QAnnotateKind::kQWeight) {
    int axis = param->axis;

    if (axis == 2) {
      reporter->Assign(types[1], TensorType({data->shape[axis], data->shape[axis+1]}, DataType::Float(32))); // Jasper: Depthwise op
    } else if (axis < 0) {
      axis = axis + data->shape.size();
      reporter->Assign(types[1], TensorType({data->shape[axis]}, DataType::Float(32)));
    } else { // Jasper: dense os
      reporter->Assign(types[1], TensorType({data->shape[axis]}, DataType::Float(32)));
    }
...
RELAY_REGISTER_OP("relay.op.annotation.simulated_quantize")
    ...
    .add_argument("dom_scale", "Tensor", "The domain scale of input data. It should be a scalar or vector") // Abraham Per-Channel
