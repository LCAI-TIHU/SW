...
Pass QuantizeAipuAnnotate() {
  ...
  runtime::TypedPackedFunc<Function(Function, IRModule, PassContext)> pass_func =
      [=](Function f, IRModule m, PassContext pc) {
        auto func = Downcast<Function>(ForwardRewrite(f, "FQAipuAnnotateRewrite", nullptr, fmulti_ref));
        auto new_params = func->params;
        for (const auto& x : FreeVars(func)) {
          new_params.push_back(x);
        }
  ...
  return CreateFunctionPass(pass_func, 1, "QuantizeAipuAnnotate", {});
}

TVM_REGISTER_GLOBAL("relay._quantize.QuantizeAipuAnnotate").set_body_typed(QuantizeAipuAnnotate);
