...
Pass QuantizeAipuPartition() {
  runtime::TypedPackedFunc<Function(Function, IRModule, PassContext)> pass_func =
      [=](Function f, IRModule m, PassContext pc) {
        auto ret = Downcast<Function>(ForwardRewrite(f, "FQAipuPartitionRewrite", nullptr, nullptr));
        return ret;
      };
  return CreateFunctionPass(pass_func, 1, "QuantizeAipuPartition", {});
}

TVM_REGISTER_GLOBAL("relay._quantize.QuantizeAipuPartition").set_body_typed(QuantizeAipuPartition);
