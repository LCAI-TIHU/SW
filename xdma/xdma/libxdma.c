...
static irqreturn_t user_irq_service(int irq, struct xdma_user_irq *user_irq)
{
	...
	if (user_irq->handler){
		printk("In user_irq handler.\n"); // shenfw add
		return user_irq->handler(user_irq->user_idx, user_irq->dev);
	}

	spin_lock_irqsave(&(user_irq->events_lock), flags);
	printk("The user_irq->events_irq = %d\n", user_irq->events_irq); // shenfw add
...
// shenfw add, the function to be executed when riscv irq arrived.
/*static irqreturn_t aipu_engine_isr(int32_t irq, void *dev_id)
{
    ...
    //spin_lock_irqsave(&xpdev->aipu_lock, flags);
    //dla_isr_handler(nvdla_dev->engine_context);
    spin_lock_irqsave(&(user_irq->events_lock), flags);
    complete(&user_irq->event_notifier);
    spin_unlock_irqrestore(&(user_irq->events_lock), flags);
    //spin_unlock_irqrestore(&xpdev->aipu_lock, flags);
			...
			struct xdma_request_cb *req, struct xdma_transfer *xfer)
{
	...
	dbg_sg("xfer= %p transfer->desc_bus = 0x%llx desc_node = %d.\n",
		xfer, (u64)xfer->desc_bus, desc_max);
		       ...
		       int *h2c_channel_max, int *c2h_channel_max)
{
	...
	if (rv < 0)
		goto err_engines;
 	printk("Before irq_setup()");
	rv = irq_setup(xdev, pdev);
	printk("After irq_setup()");
	if (rv < 0)
		goto err_msix;
