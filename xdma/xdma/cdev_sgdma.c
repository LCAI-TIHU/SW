...
static int char_sgdma_map_user_buf_to_sgl(struct xdma_io_cb *cb, bool write)
{
	...
	dbg_sg("pages_nr %u, len %lu, PAGE_SIZE %lu, PAGE_MASK 0x%lx, PAGE_SHIFT %u.\n",
			pages_nr, len, PAGE_SIZE, PAGE_MASK, PAGE_SHIFT);
...
// shenfw add the riscv irq ioctl function.
static int ioctl_do_riscv_irq(struct xdma_engine *engine, struct xdma_cdev *xcdev,unsigned long arg)
{
    int rv;
    struct xdma_dev *xdev;
    ...
    xdev = engine->xdev;
    user_irq = &xdev->user_irq[0]; // 0 for the first irq dev, pcie irq

    if (!xdev) {
        pr_err("Do the riscv irq failed. Invalid xdev\n");
        return -EINVAL;
    }

    if (!user_irq) {
    	pr_info("xdev 0x%p, user_irq NULL.\n", xdev);
        return -EINVAL;
    }


    phys = pci_resource_start(xdev->pdev, 0);
    dbg_sg("Region 0 base phys addr = %lu\n", phys);
    dbg_sg("SCU reg / RISCV irq reg offset: [hex]-0x300058, [dec]-%d.\n", 3145816);
    res_length = pci_resource_len(xdev->pdev,0);
    dbg_sg("res_length = %lu\n", res_length);
    reg = ioremap(phys + 3145816, 4);//, res_length - 3145816); // riscv irq offset: [hex](0x300058)->[dec](3145816)
    dbg_sg("reg = %p\n", reg);
    dst = (u32)arg;
    dbg_sg("rv = %d, dst = %u, arg =%lu \n", rv, dst, arg);
    iowrite32(dst, reg);
    dbg_sg("reg value = %d\n", ioread32(reg));
	...
	return -ERESTARTSYS;
     ...
     if(task_complete){
     	printk("The task complete! event_user = %d\n", events_user);
     }
...
// shenfw add the riscv irq ioctl function.
static int ioctl_do_host_irq(struct xdma_engine *engine, unsigned long arg)
{
    int rv;
    struct xdma_dev *xdev;
    unsigned long phys;
    ...
    phys = pci_resource_start(xdev->pdev, 0);
    dbg_sg("Region 0 base phys addr = %lu\n", phys);
    dbg_sg("pcie irq reg offset: [hex]-0x30005c, [dec]-%d.\n", 3145820);
    res_length = pci_resource_len(xdev->pdev,0);
    dbg_sg("res_length = %lu\n", res_length);
    reg = ioremap(phys + 3145820, 4);//, res_length - 3145820); // riscv irq offset: [hex](0x30005c)->[dec](3145820)
    dbg_sg("reg = %p\n", reg);
    dst = (u32)arg;
    dbg_sg("rv = %d, dst = %u, arg =%lu \n", rv, dst, arg);
    iowrite32(dst, reg);
		...
		unsigned long arg)
{
	...
	case IOCTL_XDMA_ALIGN_GET:
		...
		break;
	case IOCTL_XDMA_RISCV_IRQ: // shenfw add
        	rv = ioctl_do_riscv_irq(engine, xcdev, arg); // shenfw add
        	break; // shenfw add
	case IOCTL_XDMA_HOST_IRQ: // shenfw add
                rv = ioctl_do_host_irq(engine, arg); // shenfw add
                break; // shenfw add
	default:
