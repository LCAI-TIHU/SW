...
int bridge_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	phys = pci_resource_start(xdev->pdev, 0) + off; // shenfw 'xcdev->bar' -> '0'
	vsize = vma->vm_end - vma->vm_start;
	/* complete resource */
	psize = pci_resource_end(xdev->pdev, 0) -
		pci_resource_start(xdev->pdev, 0) + 1 - off;
	//psize = pci_resource_len(xdev->pdev,0); // shenfw 
	...
	rv = io_remap_pfn_range(vma, vma->vm_start, phys >> PAGE_SHIFT,
			vsize, vma->vm_page_prot); //shenfw comment out
	//
	//rv = ioremap(phys, psize);
...
/*int bridge_mmap1(struct file *file, struct vm_area_struct *vma)
{
        struct xdma_dev *xdev;
        struct xdma_cdev *xcdev = (struct xdma_cdev *)file->private_data;
        unsigned long off;
